...
class PocketApp:
    ...
    def __init__(self):
        ...
        self._pocket = Pocket(
            self._configs.get("consumer_key"), self._configs.get("access_token")
        )
    ...
    def search(self, search, state, tag, sort):
        try:
            articles = self._pocket.retrieve(
                search=search, state=state, tag=tag, sort=sort
            )
    ...
    def fetch_articles(self, output_progress=False):
        ...
        while True:
            try:
                articles = self._pocket.retrieve(
                    state="unread", count=count, offset=offset, since=last_fetch
                )
            except PocketException as e:
                spinner.finish()
                raise_from(self._check_exception(e), e)
