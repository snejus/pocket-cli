...
@click.command()
@click.option("--consumer-key", "-k", prompt="Please provide your consumer key")
@click.option(
    "--sort_field",
    "-s",
...
@click.option("--limit", "-l", default=10, help="Number of items to list")
@click.option(
    "--order",
    "-o",
    default="asc",
...
def list_articles(limit, order):
    ...
    if not articles:
        print("Articles index is empty," "run pocket-cli fetch to index your articles")
        return
...
@click.option(
    "--state", "-s", type=click.Choice(["unread", "archive", "all"]), default="unread"
)
@click.option("--tag", "-t")
@click.option(
    "--sort",
...
@click.option("--archive", "-a", is_flag=True, default=False, help="Archive article")
...
@click.option("--archive", "-a", is_flag=True, default=False, help="Archive article")
@click.option("--browser", "-b", is_flag=True, default=False, help="Open in browser")
...
def exception_occured(exception):
    print(
        "An error occured while " "trying to perform requested action: {}".format(
            exception.message
        )
    )
